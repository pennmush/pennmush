        ...
        if (global_eval_context.include_called) {
          ...
          for (a = 0; a < 10; a++) {
            if (global_eval_context.include_called == 1) {
              tmp->env[a] =
                entry->env[a] ? GC_STRDUP(entry->env[a]) : NULL;
            } else {
              if (global_eval_context.include_wenv[a]) {
                tmp->env[a] =
                  GC_STRDUP(global_eval_context.include_wenv[a]);
              } else {
                tmp->env[a] = NULL;
              }
            }
          ...
          for (a = 0; a < NUMQ; a++)
            if (!global_eval_context.renv[a] || !*global_eval_context.renv[a])
              ...
              tmp->rval[a] = GC_STRDUP(global_eval_context.renv[a]);
            }
          global_eval_context.include_called = 0;
          /* Put the included actions in the clone */
          tmp->comm =
            GC_STRDUP(global_eval_context.include_replace);
