      ...
      if (!
          (type ==
           TYPE_PLAYER ? ok_player_name(bon, player, thing) : ok_name(bon,
                                                                      1))) {
        *newalias = GC_STRDUP(bon);   /* So we can report the invalid alias */
        return OPAE_INVALID;
      }
      if (aliases > 1) {
        ...
        if (global_eval_context.pe_info->switch_nesting >= 0) {
          for (i = 0; i <= global_eval_context.pe_info->switch_nesting; i++) {
            pe_info->switch_text[i] =
              GC_STRDUP(global_eval_context.pe_info->switch_text[i]);
          }
        }
        pe_info->switch_text[i] = GC_STRDUP(expression);
