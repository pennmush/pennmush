...
main(int argc, char **argv)
{
  ...
  desc.add_options()
    ("help,h", "print help message")
	  (",z", po::value<int>(&comp)->implicit_value(COMP::GZ, "")->zero_tokens(), "compressed with gzip")
	  (",Z", po::value<int>(&comp)->implicit_value(COMP::Z, "")->zero_tokens(), "compressed with compress")
	  (",j", po::value<int>(&comp)->implicit_value(COMP::BZ2, "")->zero_tokens(), "compressed with bzip2")
	  ("inplace,i", po::bool_switch(&inplace), "update database in place")
	  ("dbref,d", po::value<dbref>(&who)->default_value(-1), "Player to modify")
	  ("all,a", po::bool_switch(&all), "Modify all players")
	  ("clear,c", po::bool_switch(&clear), "Erase password")
	  ("password,p", po::value<std::string>(&newpass)->default_value("hunter2"), "New password");
