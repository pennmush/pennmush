...
#define CMD_T_SWITCHES                                                         \
  0x02000000 /**< Any unknown or undefined switches will be passed in          \
                switches, instead of causing error */

#define CMD_T_DISABLED                                                         \
  0x01000000 /**< Command is disabled, set with \@command */
...
#define CMD_T_LISTED                                                           \
  0x00080000 /**< INTERNAL : Command is listed in \@list commands. NO LONGER   \
                USED */

#define CMD_T_INTERNAL                                                         \
  0x00040000 /**< INTERNAL : Command is an internal command, and shouldn't be  \
                matched or aliased */

#define CMD_T_LOGNAME 0x00020000 /**< Log when the command is used */
#define CMD_T_LOGARGS                                                          \
  0x00010000 /**< Log when the command is used, and the args given */

#define CMD_T_EQSPLIT                                                          \
  0x00000001 /**< Split arguments at =, but don't abort if there's no = */

#define CMD_T_ARGS 0x00000010 /**< Split into argv[] at ,s */

#define CMD_T_ARG_SPACE                                                        \
  0x00000020 /**< Split at spaces instead of commas. CMD_T_ARGS MUST also be   \
                defined */

#define CMD_T_NOPARSE 0x00000040 /**< Do NOT parse arguments */

#define CMD_T_RS_BRACE                                                         \
  0x00000080 /**< Strip outer {} from the RHS arg, even if we normally         \
                wouldn't */
 ...
 * \verbatim
   ...
   arg_right : The single arg after the =, if CMD_T_EQSPLIT is given but
 CMD_T_RSARGS isn't
   args_right : Array of args after =, if CMD_T_EQSPLIT | CMD_T_RSARGS is
 defined.
   queue_entry : The queue entry the command is being run in

   Note that if you don't specify EQSPLIT, left is still the data you want. If
 you define EQSPLIT,
   there are also arg*_right values.

   Special case:
   If the NOEVAL switch is given, AND EQSPLIT is defined, the right-side will
 not be parsed.
...
#define COMMAND(command_name)                                                  \
  void command_name(                                                           \
    COMMAND_INFO *cmd, dbref executor, dbref enactor, dbref caller,            \
    switch_mask sw, const char *raw, const char *switches, char *args_raw,     \
    char *arg_left, char *args_left[MAX_ARG], char *arg_right,                 \
    char *args_right[MAX_ARG], MQUE *queue_entry __attribute__((__unused__))); \
...
#define COMMAND_PROTO(command_name)                                            \
  void command_name(COMMAND_INFO *cmd, dbref player, dbref cause,              \
                    dbref caller, switch_mask sw, const char *raw,             \
                    const char *switches, char *args_raw, char *arg_left,      \
                    char *args_left[MAX_ARG], char *arg_right,                 \
                    char *args_right[MAX_ARG], MQUE *queue_entry)
...
struct command_info {
  ...
  struct {
    ...
    struct hook_data *ignore;   /**< Hook to evaluate to decide if we should
                                   ignore hardcoded command */
    struct hook_data *override; /**< Hook to override command with $command */
    struct hook_data
      *extend; /**< Hook to extend a command with new softcoded switches */
  } hooks;
};
...
COMMAND_INFO *command_add(const char *name, int type, const char *flagstr,
                          const char *powers, const char *switchstr,
                          command_func func);
COMMAND_INFO *make_command(const char *name, int type, const char *flagstr,
                           const char *powerstr, const char *sw,
                           command_func func);
...
void command_splitup(dbref player, dbref cause, char *from, char *to,
