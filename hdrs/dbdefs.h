...
#define Terse(x)                                                               \
  (IS(Owner(x), TYPE_PLAYER, "TERSE") || IS(x, TYPE_THING, "TERSE"))
#define Myopic(x) (IS(Owner(x), TYPE_PLAYER, "MYOPIC"))
#define Nospoof(x)                                                             \
  (IS(Owner(x), TYPE_PLAYER, "NOSPOOF") ||                                     \
   has_flag_by_name(x, "NOSPOOF", NOTYPE))
#define Paranoid(x)                                                            \
  (IS(Owner(x), TYPE_PLAYER, "PARANOID") ||                                    \
   has_flag_by_name(x, "PARANOID", NOTYPE))
...
#define ThingInhearit(x) (IS(x, TYPE_THING, "LISTEN_PARENT")) /* 0x80 */
...
#define Mistrust(x)                                                            \
  (has_flag_by_name(x, "MISTRUST", TYPE_THING | TYPE_EXIT | TYPE_ROOM))
...
#define PUSH(thing, locative) ((Next(thing) = (locative)), (locative) = (thing))

#define DOLIST_VISIBLE(var, first, player)                                     \
  for ((var) = first_visible((player), (first)); GoodObject((var));            \
       (var) = first_visible((player), Next(var)))
...
#define SPOOF(executor, enactor, sw)                                           \
  (SW_ISSET(sw, SWITCH_SPOOF) &&                                               \
       (Can_Nspemit(executor) || controls(executor, enactor))                  \
     ? enactor                                                                 \
     : executor);
...
#define AN_EVERYWHERE                                                          \
  (AN_CHAT | AN_SAY | AN_MOVE | AN_LOOK | AN_UNPARSE | AN_WHO | AN_SYS |       \
   AN_ANNOUNCE)

#define AN_TYPES (AN_PLAYER | AN_THING | AN_ROOM | AN_EXIT)

#define moniker_type(x)                                                        \
  ((IsPlayer(x) && (options.monikers & AN_PLAYER)) ||                          \
