...
#define Chanuser_Hide(u)                                                       \
  ((CUtype(u) & CU_HIDE) || (IsPlayer(CUdbref(u)) && hidden(CUdbref(u))))
...
struct channel {
  ...
  unsigned long int
    num_messages;   /**< How many messages handled by this chan since startup */
...
#define Chan_Ok_Type(c, o)                                                     \
  ((IsPlayer(o) && Channel_Player(c)) || (IsThing(o) && Channel_Object(c)))
#define Chan_Can(p, t)                                                         \
  (!(t & CHANNEL_DISABLED) && (!(t & CHANNEL_WIZARD) || Wizard(p)) &&          \
   (!(t & CHANNEL_ADMIN) || Hasprivs(p) ||                                     \
    (has_power_by_name(p, "CHAT_PRIVS", NOTYPE))))
...
#define Chan_Can_Join(c, p)                                                    \
  (Chan_Can_Access(c, p) && (eval_chan_lock(c, p, CLOCK_JOIN)))
#define Chan_Can_Speak(c, p)                                                   \
  (Chan_Can_Access(c, p) && (eval_chan_lock(c, p, CLOCK_SPEAK)))
#define Chan_Can_Cemit(c, p) (!Channel_NoCemit(c) && Chan_Can_Speak(c, p))
#define Chan_Can_Modify(c, p)                                                  \
  (Wizard(p) || (ChanCreator(c) == (p)) ||                                     \
   (!Guest(p) && Chan_Can_Access(c, p) && (eval_chan_lock(c, p, CLOCK_MOD))))
#define Chan_Can_See(c, p)                                                     \
  (Hasprivs(p) || See_All(p) ||                                                \
   (Chan_Can_Access(c, p) && (eval_chan_lock(c, p, CLOCK_SEE))) ||             \
...
void init_chat(void);
void do_channel(dbref player, const char *name, const char *target,
                const char *com);
...
int do_chat_by_name(dbref player, const char *name, const char *msg,
                    int source);
