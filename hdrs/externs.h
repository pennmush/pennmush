...
#define new_queue_actionlist(executor,enactor,caller,actionlist,parent_queue,flags,queue_type,regs) \
    ...
    ATTR *queue_attribute_getatr(dbref executor, const char *atrname,
                                 int noparent);
    ...
    int queue_include_attribute(dbref thing, const char *atrname,
                                dbref executor, dbref cause, dbref caller,
...
/* From plyrlist.c */
    ...
    void reset_player_list(dbref player, const char *oldname,
                           const char *oldalias, const char *name,
                           const char *alias);
...
/** Errors from ok_player_alias */
    ...
    enum opa_error ok_player_alias(const char *alias, dbref player,
                                   dbref thing);
    enum opa_error ok_object_name(char *name, dbref player, dbref thing,
                                  int type, char **newname, char **newalias);
