...
#define LF_NOSUCCACTION                                                        \
  0x040U /**< This lock doesn't have an \@a-action for success. */
#define LF_NOFAILACTION                                                        \
  0x080U                /**< This lock doesn't have an \@a-action for failure */
...
lock_type check_lock_type(dbref player, dbref thing, lock_type name,
                          bool silent);
int add_lock(dbref player, dbref thing, lock_type type, boolexp key,
             privbits flags);
int add_lock_raw(dbref player, dbref thing, lock_type type, boolexp key,
                 privbits flags);
void free_locks(lock_list *ll);
int eval_lock_with(dbref player, dbref thing, lock_type ltype,
                   NEW_PE_INFO *pe_info);
#define eval_lock(player, thing, ltype)                                        \
...
#define Can_Read_Lock(p, x, l)                                                 \
  (See_All(p) || controls(p, x) ||                                             \
   ((Visual(x) || lock_visual(x, l)) && eval_lock(p, x, Examine_Lock)))
...
extern lock_type
  InFilter_Lock; /* Whose sound is played inside listening objects */
extern lock_type DropIn_Lock; /* Who can drop objects in a location */
extern lock_type Chown_Lock;  /* Who can @chown me if I'm chown_ok? */
