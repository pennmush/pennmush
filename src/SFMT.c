 ...
 */
static void
period_certification(sfmt_t *sfmt)
{
  uint32_t inner = 0;
  int i, j;
  uint32_t work;
 ...
 */
int
sfmt_get_min_array_size32(sfmt_t *sfmt)
{
  UNUSED_VARIABLE(sfmt);
  return SFMT_N32;
}
 ...
 */
int
sfmt_get_min_array_size64(sfmt_t *sfmt)
{
  UNUSED_VARIABLE(sfmt);
  return SFMT_N64;
}
 ...
 */
void
sfmt_fill_array32(sfmt_t *sfmt, uint32_t *array, int size)
{
 ...
 */
void
sfmt_fill_array64(sfmt_t *sfmt, uint64_t *array, int size)
{
  assert(sfmt->idx == SFMT_N32);
  assert(size % 2 == 0);
 ...
 */
void
sfmt_init_gen_rand(sfmt_t *sfmt, uint32_t seed)
{
  int i;
