...
#define DEBUG_CHUNK_REGION_CREATE
/** Log paging of regions. */
#undef DEBUG_CHUNK_PAGING
/** Log all mallocs. */
#define DEBUG_CHUNK_MALLOC
...
#endif
    rhp = GC_MALLOC_UNCOLLECTABLE(REGION_SIZE);
    if (!rhp) {
...
#endif
  regions = GC_MALLOC_UNCOLLECTABLE(region_array_len * sizeof(Region));
  if (!regions)
