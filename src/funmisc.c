...
paren_match(char *atext, char *buff, char **bptr, int key, int i_type,
            int i_indent, int i_extra)
{
  char *atextptr;
  int oldtcnt, tcnt, i_clr, i_err, esc_val, i_pos, i_indentnow, i_close,
    i_lastclose, i_space;
  static int iarr[BUFFER_LEN];
  static char *s_clrs[] = {ANSI_MAGENTA, ANSI_GREEN, ANSI_YELLOW, ANSI_CYAN,
                           ANSI_BLUE};

  i_indentnow = i_pos = i_err = i_clr = tcnt = oldtcnt = esc_val = i_close =
    i_lastclose = 0;
  atextptr = atext;
  while (*atextptr) {
    i_lastclose = i_close;
