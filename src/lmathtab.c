...
/* Command-line: /usr/pkg/bin/gperf -C --output-file lmathtab.c lmathtab.gperf  */
...
#endif
static unsigned int
math_hash (register const char *str, register size_t len)
{
...
const struct math *
math_hash_lookup (register const char *str, register size_t len)
{
  enum
    {
      TOTAL_KEYWORDS = 31,
...
#line 74 "lmathtab.gperf"
  ...
  if (len <= MAX_WORD_LENGTH && len >= MIN_WORD_LENGTH)
    {
      register unsigned int key = math_hash (str, len);
