...
as_get_tag(ansi_string *as, const char *tag)
{
  ...
  if (as->tags == NULL) {
    as->tags = GC_MALLOC(sizeof(StrTree));
...
grow_mi(ansi_string *as, char type)
{
  if (as->micount >= as->misize) {
    if (as->mi == NULL) {
      as->misize = 30;
      as->mi = GC_MALLOC(as->misize * sizeof(new_markup_information));
    } else {
      as->misize *= 2;
      as->mi = GC_REALLOC(as->mi,
...
parse_ansi_string(const char *source)
{
  ...
  as->source = GC_STRDUP(source);

  /* The string has markup. Nuts. */
  as->flags |= AS_HAS_MARKUP;
  as->markup = GC_MALLOC_ATOMIC(sizeof(uint16_t) * BUFFER_LEN);
...
ansi_string_replace(ansi_string *dst, int loc, int count, ansi_string *src)
{
  ...
  if (src->len < 1) {
    ...
    if (src->markup && src->flags & AS_HAS_STANDALONE) {
      ...
      if (!dst->markup) {
        dst->markup = GC_MALLOC_ATOMIC(sizeof(uint16_t) * BUFFER_LEN);
        for (i = 0; i < dst->len; i++) {
  ...
  if (!src->markup) {
    src->markup = GC_MALLOC_ATOMIC(sizeof(uint16_t) * BUFFER_LEN);
    for (i = 0; i < srclen; i++) {
