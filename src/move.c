 ...
 */
void
moveit(dbref what, dbref where, int nomovemsgs, dbref enactor,
       const char *cause)
{
  dbref loc, old;
  dbref absloc, absold;
  ...
  if (!WIZ_NOAENTER || !(Wizard(what) && DarkLegal(what)))
    if ((where != NOTHING) && (old != where)) {
      did_it_with(what, what, NULL, NULL, "OXMOVE", NULL, NULL, old, where, old,
                  NA_INTER_HEAR, AN_MOVE);
      if (Hearer(what)) {
  ...
  if (!nomovemsgs)
    did_it_with(what, what, "MOVE", NULL, "OMOVE", NULL, "AMOVE", where, where,
                old, NA_INTER_SEE, AN_MOVE);
  queue_event(enactor, "OBJECT`MOVE", "%s,%s,%s,%d,%s", unparse_objid(what),
              unparse_objid(where), unparse_objid(old), nomovemsgs ? 1 : 0,
              cause);
}
 ...
 */
void
enter_room(dbref player, dbref loc, int nomovemsgs, dbref enactor,
           const char *cause)
{
  dbref old;
  dbref dropto;
  ...
  if ((loc != old) && Dropper(player) && (old != NOTHING) && (IsRoom(old)) &&
 ...
 */
void
safe_tel(dbref player, dbref dest, int nomovemsgs, dbref enactor,
         const char *cause)
{
  dbref first;
  dbref rest;
...
can_move(dbref player, const char *direction)
{
  ...
  if (!strcasecmp(direction, "home")) {
    ...
    ok = (match_result(player, direction, TYPE_EXIT,
      ...
      if (could_doit(player, exit_m, pe_info)) {
        ...
        case AMBIGUOUS:
          ...
          if (!GoodObject(var_dest) ||
              !can_link_to(exit_m, var_dest, pe_info)) {
            notify_format(
              player,
              T("Variable exit destination #%d is invalid or not permitted."),
              var_dest);
            return;
          }
          break;
        ...
        did_it_with(player, exit_m, "SUCCESS", NULL, "OSUCCESS", NULL,
                    "ASUCCESS", NOTHING, Location(player), NOTHING,
                    NA_INTER_HEAR, AN_MOVE);
...
do_firstexit(dbref player, const char **what)
{
  ...
  for (i = 1; i < MAX_ARG && what[i]; i++) {
    if ((thing = noisy_match_result(player, what[i], TYPE_EXIT,
                                    MAT_ENGLISH | MAT_EXIT | MAT_TYPE)) ==
        NOTHING)
      continue;
    loc = Home(thing);
    ...
    notify_format(player, T("%s is now the first exit in %s."),
  ...
  if (match_result(player, what, TYPE_THING, match_flags) == NOTHING) {
    if (POSSESSIVE_GET) {
      ...
      if (could_doit(player, thing, pe_info) &&
          (POSSGET_ON_DISCONNECTED || (!IsPlayer(box) || Connected(box))) &&
        ...
        notify_format(box, T("%s was taken from you."), stolen);
        ...
        *tp = '\0';
        ...
        did_it_with(player, thing, "SUCCESS", tbuf1, "OSUCCESS", tbuf2,
                    "ASUCCESS", NOTHING, box, NOTHING, 0, AN_MOVE);
    ...
    if ((thing = noisy_match_result(player, what, TYPE_THING, match_flags)) !=
      ...
      case TYPE_THING:
        ...
        if (could_doit(player, thing, pe_info)) {
          ...
          tp = tbuf1;
          safe_format(tbuf1, &tp, T("You take %s."),
...
do_drop(dbref player, const char *name, NEW_PE_INFO *pe_info)
{
  ...
  if (!Mobile(player))
    return;
  switch (thing = match_result(player, name, TYPE_THING | TYPE_PLAYER,
  ...
  default:
    if (Location(thing) != player) {
      ...
      return;
               ...
               !eval_lock_with(player, loc, Drop_Lock, pe_info)) {
...
do_empty(dbref player, const char *what, NEW_PE_INFO *pe_info)
{
  ...
  if (!Mobile(player))
    return;
  thing = noisy_match_result(player, what, TYPE_THING | TYPE_PLAYER,
    ...
    if (empty_ok) {
      ...
      if (thing != player) {
        ...
        *tp = '\0';
        ...
        did_it_with(player, item, "SUCCESS", tbuf1, "OSUCCESS", tbuf2,
                    "ASUCCESS", NOTHING, thing_loc, NOTHING, NA_INTER_HEAR,
                    AN_MOVE);
        did_it_with(player, player, "RECEIVE", NULL, "ORECEIVE", NULL,
  ...
  if (count == 1)
    notify_format(player, T("You remove 1 object from %s."),
                  AName(thing, AN_MOVE, NULL));
  else
    notify_format(player, T("You remove %d objects from %s."), count,
                  AName(thing, AN_MOVE, NULL));
...
do_leave(dbref player, NEW_PE_INFO *pe_info)
{
  ...
  if (IsRoom(loc) || IsGarbage(loc) || IsGarbage(Location(loc)) ||
      NoLeave(loc) || !eval_lock_with(player, loc, Leave_Lock, pe_info)) {
...
do_follow(dbref player, const char *arg, NEW_PE_INFO *pe_info)
{
  ...
  if (arg && *arg) {
    ...
    if (is_following(player, leader)) {
      notify_format(player, T("You're already following %s."),
                    AName(leader, AN_SYS, NULL));
      return;
    }
    /* Ok, are we allowed to follow them? */
    if (!eval_lock_with(player, leader, Follow_Lock, pe_info)) {
      fail_lock(player, leader, Follow_Lock, T("You're not allowed to follow."),
                Location(player));
      return;
    }
    /* Ok, looks good */
...
do_unfollow(dbref player, const char *arg)
{
  ...
  if (arg && *arg) {
    ...
    if (!is_following(player, leader)) {
      notify_format(player, T("You're not following %s."),
                    AName(leader, AN_SYS, NULL));
      return;
    }
    /* Ok, looks good */
...
do_dismiss(dbref player, const char *arg)
{
  ...
  if (arg && *arg) {
    ...
    if (!is_following(follower, player)) {
      notify_format(player, T("%s isn't following you."),
...
clear_followers(dbref leader, int noisy)
{
  ...
  while (s) {
    ...
    if (GoodObject(flwr)) {
      ...
      if (noisy)
        notify_format(flwr, T("You stop following %s."),
                      AName(leader, AN_SYS, NULL));
    }
  }
...
clear_following(dbref follower, int noisy)
{
  ...
  while (s) {
    ...
    if (GoodObject(ldr)) {
      ...
      if (noisy)
        notify_format(ldr, T("%s stops following you."),
                      AName(follower, AN_SYS, NULL));
    }
  }
