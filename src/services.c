...
main(int argc, char **argv)
{
  ...
  SERVICE_TABLE_ENTRY dispatchTable[] = {
    {THIS_SERVICE, (LPSERVICE_MAIN_FUNCTION) service_main}, {NULL, NULL}};
  ...
  if ((argc == 2) &&
    ...
    if (!StartServiceCtrlDispatcher(dispatchTable)) {
      fprintf(stderr, "Unable to start service, assuming running console-mode "
                      "application.\n");
      fprintf(stderr, "You can save time on the next invocation by specifying: "
                      "pennmush /run\n");
      worker_thread(NULL);
    }
  } /*  end of argc == 1 */
                    ...
                    DWORD dwCheckPoint, DWORD dwWaitHint)
{
  ...
  if (!(fResult =
          SetServiceStatus(sshStatusHandle, /*  service reference handle */
...
CmdInstallService(int argc, char *argv[])
{
  ...
  service = CreateService(SCmanager, THIS_SERVICE, THIS_SERVICE_DISPLAY,
                          SERVICE_ALL_ACCESS, SERVICE_WIN32_OWN_PROCESS,
                          SERVICE_AUTO_START, SERVICE_ERROR_NORMAL,
                          fullfilename, NULL, /*  no load ordering group */
