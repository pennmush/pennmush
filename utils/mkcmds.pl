...
sub make_gitinfo {
    print "Rebuilding gitinfo.h header\n";
    my ($HDR, $temphdr) = tempfile("gitinfoXXXXX", SUFFIX => ".h", TMPDIR => 1);
    push @tmpfiles, $temphdr;
    
    print $HDR "/* AUTOGENERATED FILE. DO NOT EDIT! */\n";

    my $version = `git rev-parse --short HEAD`;
    chomp $version;
    
    print $HDR '#define GIT_REVISION "', $version, '"', "\n";
    close $HDR;
    maybemove $temphdr, "hdrs/gitinfo.h";
}
